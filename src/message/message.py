from src.model import MessageType, Chatroom, ChatroomMember, Friend, MessageSource, XmlType
from src.manager import cache
from src.utils import logger
from src.bot import Bot
from .entity import Quote, File
...
class TextMessage(Message):
    ...
    async def parse(self) -> "TextMessage":
        ...
        # if self.sender.wxid == "wxid_s53l3b64dulc22":
        #     return None # TODO
        return self
...
class XmlMessage(TextMessage):
    ...
    async def parse(self) -> "XmlMessage":
        ...
        if type == XmlType.QUOTE:
            return await self.handle_quote()
        elif type == XmlType.FILE:
            return logger.warning("未适配FILE类型XML")
        elif type == XmlType.UPLOAD:  # 文件消息，但还在上传，不用管
            return logger.warning("未适配UPLOAD类型XML")

        else:
            logger.info("未知的xml消息类型: {}", type)
        return self
