...
async def create_game(message: TextMessage):
...
async def vote(message: TextMessage):
    chatroom_id = message.chatroom.chatroom_id
    if session := GameSession.get(chatroom_id):
        if len(message.ats) != 2:
            return await bot.send_text(chatroom_id, "格式错误", message.sender.wxid)
        at = next(wxid for wxid in message.ats if wxid != bot.wxid)
